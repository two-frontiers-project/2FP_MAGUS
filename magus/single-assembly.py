    ...
    def run_megahit(self, sample):
        ...
        if pd.isna(r2) or r2 is None:
            cmd = (f"megahit-g --k-min 75 --k-max 333 --k-step 6 --cleaning-rounds 1 "
                   ...
                   f"--continue -t {self.threads} -r {r1} -o {out_sample_dir}")
            print(f"Running MEGAHIT in single-end mode on {sample_name}")
        else:
            cmd = (f"megahit-g --k-min 75 --k-max 333 --k-step 6 --cleaning-rounds 1 "
                   f"--merge-level 100,.999 --min-count 1 --min-contig-len 1000 "
                   f"--continue -t {self.threads} -1 {r1} -2 {r2} -o {out_sample_dir}")
            print(f"Running MEGAHIT in paired-end mode on {sample_name}")
