...
class AssembleHost:
    ...
    def load_config(self, config_path):
        """Loads the config file and ensures empty pe2 values are properly handled."""
        config_df = pd.read_csv(config_path, sep='\t')
        config_df['pe2'] = config_df['pe2'].apply(lambda x: x if pd.notna(x) else None)
    ...
    def generate_host_assembly_config(self):
        ...
        new_config.to_csv(self.output_config, sep='\t', index=False, na_rep='')
