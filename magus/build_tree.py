#!/usr/bin/env python3
"""
Build phylogenetic trees from single copy genes.

This script takes a gene list and ORF summary and:
...
def run_fasttree(aligned_file: str, output_dir: str, threads: int = 1) -> str:
    ...
    tree_file = os.path.join(output_dir, "phylogeny.tree")
...
def run_iqtree(aligned_file: str, output_dir: str, threads: int = 1) -> str:
    ...
    tree_prefix = os.path.join(output_dir, "phylogeny")
    cmd = ['iqtree2', '-s', aligned_file, '-nt', str(threads), '-pre', tree_prefix]
...
def main():
    parser = argparse.ArgumentParser(
        description='Build phylogenetic trees from concatenated single copy genes'
    )
    parser.add_argument(
        'fasta_dir',
    ...
    parser.add_argument(
        '--gene-list',
        required=True,
        help='Path to single copy genes file'
    )
    parser.add_argument(
        '--orf-data',
        required=True,
        help='Path to ORF summary file'
    )
    parser.add_argument(
        '--coverage-threshold',
        type=float,
    ...
    parser.add_argument(
        '--output-dir',
        default='phylogenetic_trees',
        help='Output directory for trees and alignments (default: phylogenetic_trees)'
    )
    parser.add_argument(
        '--genome-list',
    ...
    try:
        ...
        gene_families = load_single_copy_genes(args.gene_list)
        
        # Load ORF summary
        logger.info("Loading ORF summary...")
        samples_data = load_orf_summary(args.orf_data, genome_list)
